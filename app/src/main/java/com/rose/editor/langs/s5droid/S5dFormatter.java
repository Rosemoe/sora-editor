...
public class S5dFormatter {
    ...
    public void format() {
        ...
        Tokens lastToken = NEWLINE;
        Tokens currentToken;

        Tokens cacheToken = NEWLINE;
        String cacheTokenContent = null;

        while((currentToken = (cacheToken != NEWLINE ? cacheToken : lexer.nextToken())) != EOF) {
            //Do not call yytext() for too many times.It will cost more time to create new String
            String content;
            if(cacheToken != NEWLINE) {
            ...
            if(currentToken == LONG_COMMENT || currentToken == LINE_COMMENT) {
            ...
            if(style0 && (lastToken != LINE_COMMENT && lastToken != LONG_COMMENT)) {
                //Ensure the space will only be added once
                if(isOperator(currentToken) && lastToken != WHITESPACE && lastToken != NEWLINE) {
                    sb.append(' ');
                } else if(currentToken != WHITESPACE && currentToken != NEWLINE && !isSeparator(currentToken) && !isOperator(currentToken) && isOperator(lastToken)) {
                    sb.append(' ');
                } else if(lastToken == COMP && currentToken != WHITESPACE) {
                    sb.append(' ');
                }
            }
            ...
            if(currentToken == NEWLINE) {
                sb.append('\n');
                Tokens token;
                while((token = lexer.nextToken()) == WHITESPACE || token == NEWLINE) {
            ...
            //Make advances
            {
                ...
                switch(currentToken) {
                    ...
                    case TRY:
                        ...
                        layer++;
                        if(currentToken == WHILELOOP || currentToken == FORLOOP) {
                            stack.push(LOOP);
                        } else if(currentToken == TRY) {
                            stack.push(SIMPLE_TRY);
                        } else {
                ...
                if(currentToken == COLON && !cases.isEmpty()) {
                ...
                if(currentToken == END) {
                    ...
                    Tokens token;
                    while((token = lexer.nextToken()) == WHITESPACE) {
                        if(style4) {
