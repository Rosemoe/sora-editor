...
/**
 * RoseEditor is a editor that can highlight texts region by doing grammar analyzing
 *
...
public class RoseEditor extends View implements ContentListener,TextColorProvider.Callback {
    ...
    public void setHighlightCurrentBlock(boolean mHighlightCurrentBlock){
        this.mHighlightCurrentBlock = mHighlightCurrentBlock;
    ...
    public void setPaintLabel(boolean mPaintLabel){
        this.mPaintLabel = mPaintLabel;
        invalidate();
    }
    ...
    private void prepare(){
        ...
        try
        {
            BufferedReader br = new BufferedReader(new InputStreamReader(
                    new FileInputStream("/sdcard/Test.java")));
            String b;
    ...
    private void drawView(Canvas canvas){
        ...
        //Toast.makeText(getContext(),mSpanner.getColors().mExtra != null ?((S5droidTree)mSpanner.getColors().mExtra).toTString() : "" ,0).show();
    ...
    private void drawScrollBarVertical(Canvas canvas,int width) {
        ...
        if(length < mDpUnit * 30) {
            mVerticalScrollBarSizeEnlarged = true;
            length = mDpUnit * 30;
            topY = (getOffsetY() + page / 2) / all * (getHeight() - length);
        }else{
            topY = getOffsetY() / all * getHeight();
            mVerticalScrollBarSizeEnlarged = false;
        }
        if(mEventHandler.holdVerticalScrollBar()) {
            float centerY = topY + length / 2;
    ...
    private void drawLineInfoPanel(Canvas canvas,float centerY,float rightX) {
        ...
        String text = mLnTip + Integer.toString((2 + getFirstVisibleLine() + getLastVisibleLine()) / 2);
        float textWidth = mPaint.measureText(text);
        mRect.top = centerY - getLineHeight() / 2 - expand;
        mRect.bottom = centerY + getLineHeight() / 2 + expand;
        mRect.right = rightX;
    ...
    private void drawLineBackground(Canvas canvas, int color,int line){
        int curr = line;
        if(!isLineVisible(curr)){
            return;
        }
        mRect.top = getLineTop(curr) - getOffsetY();
        mRect.bottom = getLineBottom(curr) - getOffsetY();
        mRect.left = 0;
    ...
    public void setSelectionRegion(int lineLeft, int columnLeft, int lineRight, int columnRight,boolean makeRightVisible){
        ...
        if(columnLeft > 0) {
            ...
            if(isEmoji(ch)) {
                columnLeft++;
                if(columnLeft > mText.getColumnCount(line)) {
                    columnLeft--;
                }
            }
        }
        if(columnRight > 0) {
            ...
            if(isEmoji(ch)) {
                columnRight++;
                if(columnRight > mText.getColumnCount(line)) {
                    columnRight--;
                }
            }
        }
    ...
    public void copyText(){
        try{
            if(mCursor.isSelected()){
                mClipboardManager.setText(getText().subContent(mCursor.getLeftLine(),
    ...
    public void setTextSize(float textSize){
        ...
        setTextSizePixel(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSize, res.getDisplayMetrics()));
    }

    /**
     * Set the editor's text size. This value must be > 0
     *
     * @param textSizePixel et the editor's text size in <strong>Px</strong> units.
     */
    public void setTextSizePixel(float textSizePixel){
        mPaint.setTextSize(textSizePixel);
    ...
    protected boolean hasClipData() {
        return getTextFromClipboard() != null;
    }

    protected CharSequence getTextFromClipboard() {
    ...
    public boolean onGenericMotionEvent(MotionEvent event)
    {
        switch(event.getAction()){
            case MotionEvent.ACTION_SCROLL:
                ...
                if(vscroll != 0) {
                    mEventHandler.onScroll(event, event, 0, vscroll * 20);
                }
                break;
        }
    ...
    public void afterInsert(Content content, int startLine, int startColumn, int endLine, int endColumn, CharSequence insertedContent) {
        ...
        if(getWidth() < 500 * mDpUnit) {
            ...
            mACPanel.setExtendedX(getWidth() / 8 / 2);
        }else{
