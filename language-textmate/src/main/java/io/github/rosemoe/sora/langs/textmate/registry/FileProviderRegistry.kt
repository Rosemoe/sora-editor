...
class FileProviderRegistry private constructor() {
    private val allFileResolvers = arrayListOf(FileResolver.DEFAULT)
    ...
    fun dispose() {
        allFileResolvers.forEach { it.dispose() }
        allFileResolvers.clear()
    }

    companion object {
        private var instance: FileProviderRegistry? = null

        @JvmStatic
        @Synchronized
        fun getInstance(): FileProviderRegistry {
            if (instance == null) {
                instance = FileProviderRegistry()
            }
            return instance!!
        }
    }
